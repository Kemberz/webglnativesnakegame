...
export default class SnakeGame {
  ...
  moveSnake() {
    ...
    if (upKeys.includes(lastPressedKey)) {
      ...
      newCoords.x = oldHeadXCoord + this.snakeSpeed;
      this.animateSnakeMovement(oldCoords, newCoords);
    } else if (escapeKeys.includes(lastPressedKey)){
      this.lastPressedKey = '';
      this.PauseUnpause();
    } 
    ...
    // To add skull functionality
    if (
      this.almostEqual(newCoords.x, 8) ||
      this.almostEqual(newCoords.y, 8)
    ) {
      console.log("dead")
    }
  ...
  extendSnake(lastSnakePartCoords) {
    const snakePartGeometry = new THREE.BoxGeometry(1, 1, 0.01);
